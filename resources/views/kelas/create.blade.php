<x-app-layout>
    ...
    <x-card class="max-w-2xl mx-auto">
        <form method="POST" action="{{ route('kelas.store') }}" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-semibold uppercase text-navy-700 mb-1">Nama Kelas (contoh: X IPA 1)</label>
                <input type="text" name="nama_kelas" value="{{ old('nama_kelas') }}" required class="w-full text-sm rounded-lg border-[var(--border)] bg-[var(--paper)] text-[var(--ink-900)]">
                @error('nama_kelas') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-xs font-semibold uppercase text-navy-700 mb-1">Jurusan (contoh: MIPA / IPS / Rekayasa Perangkat Lunak)</label>
                <input type="text" name="jurusan" value="{{ old('jurusan') }}" required class="w-full text-sm rounded-lg border-[var(--border)] bg-[var(--paper)] text-[var(--ink-900)]">
