...
<body class="font-sans antialiased text-[var(--ink-900)] bg-[var(--paper)]">
    <div x-data="{ sidebarOpen: false, sidebarCollapsed: false, logoutOpen: false }" class="min-h-screen flex flex-col min-[820px]:flex-row">
        ...
        <div
            ...
            class="min-[820px]:hidden fixed inset-0 z-30 bg-navy-900/60"
        ></div>

        <!-- Sidebar Navigation (230px on desktop, slide-in drawer on mobile) -->
        <aside
            :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full min-[820px]:translate-x-0'"
            class="fixed inset-y-0 left-0 z-40 min-[820px]:sticky min-[820px]:inset-auto min-[820px]:top-0 min-[820px]:min-h-screen min-[820px]:flex-shrink-0 flex flex-col justify-between bg-navy-900 text-white transition-transform duration-300 ease-in-out overflow-hidden"
            :style="sidebarCollapsed ? 'width: 64px' : 'width: 230px'"
            style="width: 230px"
        >
            <div class="flex-1 min-h-0 flex flex-col">
                <!-- Brand Title + Collapse Button (desktop) -->
                <div class="hidden min-[820px]:flex items-center justify-between px-4 py-5 border-b border-navy-700 flex-shrink-0">
                ...
                <!-- Brand Title + Close Button (mobile) -->
                <div class="min-[820px]:hidden flex items-center justify-between px-4 py-4 border-b border-navy-700 flex-shrink-0">
                    <h1 class="font-serif font-semibold text-xl tracking-tight text-white whitespace-nowrap">Sistem Sekolah</h1>
                    <button @click="sidebarOpen = false" class="p-1.5 text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-navy-700/50" title="Tutup Menu">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>

                <!-- Navigation Links (scrolls independently so footer stays pinned) -->
                <nav class="flex-1 min-h-0 overflow-y-auto mt-2 px-2 pb-4 space-y-1">

                    <p x-show="!sidebarCollapsed" class="px-3 pt-3 pb-1 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Home</p>
                    <a href="{{ route('dashboard') }}" @click="sidebarOpen = false" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('dashboard') ? 'bg-navy-700 text-white' : 'text-gray-300 hover:bg-navy-700/50 hover:text-white' }}" :title="sidebarCollapsed ? 'Dashboard' : ''">
                    ...
                    @if(Auth::user()->isAdmin() || Auth::user()->isGuru())
                        <p x-show="!sidebarCollapsed" class="px-3 pt-4 pb-1 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Data Master</p>
                        <a href="{{ route('siswa.index') }}" @click="sidebarOpen = false" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('siswa.*') ? 'bg-navy-700 text-white' : 'text-gray-300 hover:bg-navy-700/50 hover:text-white' }}" :title="sidebarCollapsed ? 'Data Siswa' : ''">
                        ...
                        @if(Auth::user()->isAdmin())
                            <a href="{{ route('guru.index') }}" @click="sidebarOpen = false" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('guru.*') ? 'bg-navy-700 text-white' : 'text-gray-300 hover:bg-navy-700/50 hover:text-white' }}" :title="sidebarCollapsed ? 'Manajemen Guru' : ''">
                                <svg class="w-5 h-5 flex-shrink-0" :class="sidebarCollapsed ? '' : 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                                <span x-show="!sidebarCollapsed" class="whitespace-nowrap">Manajemen Guru</span>
                            </a>
                        @endif

                        <a href="{{ route('kelas.index') }}" @click="sidebarOpen = false" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('kelas.*') ? 'bg-navy-700 text-white' : 'text-gray-300 hover:bg-navy-700/50 hover:text-white' }}" :title="sidebarCollapsed ? 'Data Kelas' : ''">
                    ...
                    <p x-show="!sidebarCollapsed" class="px-3 pt-4 pb-1 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Akademik</p>
                    <a href="{{ route('nilai.index') }}" @click="sidebarOpen = false" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('nilai.*') ? 'bg-navy-700 text-white' : 'text-gray-300 hover:bg-navy-700/50 hover:text-white' }}" :title="sidebarCollapsed ? 'Nilai Akademik' : ''">
                    ...
                    <p x-show="!sidebarCollapsed" class="px-3 pt-4 pb-1 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Layanan</p>
                    <a href="{{ route('pengajuan.index') }}" @click="sidebarOpen = false" class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors {{ request()->routeIs('pengajuan.*') ? 'bg-navy-700 text-white' : 'text-gray-300 hover:bg-navy-700/50 hover:text-white' }}" :title="sidebarCollapsed ? 'Pengajuan' : ''">
            ...
            <!-- User Info & Logout: pinned to the very bottom of the sidebar -->
            <div class="flex-shrink-0 p-3 border-t border-navy-700 bg-navy-900">
                <div class="flex items-center" :class="sidebarCollapsed ? 'justify-center' : 'justify-between'">
                    ...
                    <span class="inline-flex">
                        <button type="button" @click="logoutOpen = true" class="p-1.5 text-gray-400 hover:text-red-400 transition-colors" title="Logout">
        ...
        {{-- Modal Konfirmasi Logout: lives outside <aside> so `fixed inset-0` covers the whole viewport, not just the sidebar's transformed box --}}
        <div
            x-show="logoutOpen"
            x-cloak
            x-transition:enter="ease-out duration-200"
        ...
        >
            <div class="absolute inset-0 bg-navy-900/50" @click="logoutOpen = false"></div>
            <div
                x-show="logoutOpen"
                x-transition:enter="ease-out duration-200"
            ...
            >
                ...
                <div class="flex gap-3">
                    <button type="button" @click="logoutOpen = false" class="flex-1 px-4 py-2 text-sm font-medium rounded-lg border border-navy-700 text-navy-700 hover:bg-navy-100 transition-colors">
                        Tidak
                    </button>
                    <button type="button" @click="logoutOpen = false; document.getElementById('logout-form').submit()" class="flex-1 px-4 py-2 text-sm font-medium rounded-lg bg-navy-900 text-white hover:bg-navy-700 transition-colors">
                        Ya, Keluar
                    </button>
                </div>
            </div>
